...
class IXIBrain(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.train_cond_diffusion == True:
            ...
            # file_path = os.path.join(self.save_dir, f"gt.png")
            # micro_for_plot = micro
            # micro_for_plot = micro_for_plot.squeeze().detach().cpu().numpy()
            # plt.imsave(file_path, micro_for_plot, cmap='gray')
