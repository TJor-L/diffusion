...
def ftran(y, smps, mask):
    ...
    # mask^H
    # print(f"[ftran] mask.shape: {mask.shape}")
    if len(y) == 2:
...
def fmult(x, smps, mask):
    ...
    # print(f"fastMRI.py mask.shape: {mask.shape}")
...
class fastMRI(Dataset):
    ...
    def __getitem__(self, item):
        ...
        """
        x_max = np.max(np.abs(x))  # Maximum magnitude of the complex array
        x = x / x_max  # Normalize the magnitudes to [-1, 1] by dividing by max magnitude
        ...
        # y_hat = fmult(x_hat, smps_hat, mask)
        # x_hat2 = ftran(y_hat, smps_hat, mask)
        # y_hat = fmult(x_hat2, smps_hat, mask)
        # x_hat3 = ftran(y_hat, smps_hat, mask)
        y_hat = fmult_non_mask(x, smps_hat)
        x_hat = ftran_non_mask(y_hat, smps_hat)
        y_hat = fmult_non_mask(x_hat, smps_hat)
        x_hat2 = ftran_non_mask(y_hat, smps_hat)
        y_hat = fmult_non_mask(x_hat2, smps_hat)
        x_hat3 = ftran_non_mask(y_hat, smps_hat)

        x_hat = torch.view_as_real(x_hat).permute([0, 3, 1, 2]).contiguous()
        x_hat2 = torch.view_as_real(x_hat2).permute([0, 3, 1, 2]).contiguous()
        x_hat3 = torch.view_as_real(x_hat3).permute([0, 3, 1, 2]).contiguous()

        # print(f"x.shape:{x.shape}")
        x = x.unsqueeze(0)
        x_gt = torch.view_as_real(x).permute([0, 3, 1, 2]).contiguous()
        
        
        fig, axes = plt.subplots(1, 4, figsize=(20, 5))
        x_hat_for_plot = abs_helper(x_hat).squeeze().detach().cpu().numpy()
        x_hat2_for_plot = abs_helper(x_hat2).squeeze().detach().cpu().numpy()
        x_hat3_for_plot = abs_helper(x_hat3).squeeze().detach().cpu().numpy()
        x_gt_for_plot = abs_helper(x_gt).squeeze().detach().cpu().numpy()
        
        print(f"compute_metrics(reconstructed = x_gt, reference = x_hat_for_plot):{compute_metrics(reconstructed = x_gt_for_plot, reference = x_hat_for_plot)}")
        print(f"compute_metrics(reconstructed = x_gt, reference = x_hat2_for_plot):{compute_metrics(reconstructed = x_gt_for_plot, reference = x_hat2_for_plot)}")
        print(f"compute_metrics(reconstructed = x_gt, reference = x_hat3_for_plot):{compute_metrics(reconstructed = x_gt_for_plot, reference = x_hat3_for_plot)}")
        
        # def compute_metrics(reconstructed, reference, gpu=None, loss_fn=None, mode = None):
        
        # x_start_for_plot = np.clip(x_start_for_plot, 0, 1)
        # print(f"[abs_helper] max(x_start_for_plot): {np.max(x_start_for_plot)}\nmin(x_start_for_plot): {np.min(x_start_for_plot)}")
        axes[0].imshow(x_hat_for_plot, cmap='gray')
        axes[0].set_title('one')
        axes[0].axis('off')

        axes[1].imshow(x_hat2_for_plot, cmap='gray')
        axes[1].set_title(f'two')
        axes[1].axis('off')
        
        axes[2].imshow(x_hat3_for_plot, cmap='gray')
        axes[2].set_title(f'three')
        axes[2].axis('off')

        mask_for_plot = mask.squeeze().detach().cpu().numpy()
        axes[3].imshow(mask_for_plot, cmap='gray')
        axes[3].set_title('Mask')
        axes[3].axis('off')
        plt.savefig(os.path.join(".", "fmult_ftran.png"))
        plt.close(fig)
