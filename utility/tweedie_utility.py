...
def mkdir_exp_recording_folder(save_dir, acc_rate, dataset_name, compute_vb_loss, mask_pattern, image_size):
    """
    ...
    unique_name = f"{current_date}_{current_hour_minute}_{dataset_name}_vb{compute_vb_loss}_{mask_pattern}_acc{acc_rate}_img{image_size}"
...
def compute_metrics(reconstructed, reference):
    """Compute PSNR, LPIPS, and DC distance between the reconstructed and reference images."""
    ...
    # PSNR
    # raise ValueError(f"Check the range of the input image: reconstructed_np: min{reconstructed_np.min()} max{reconstructed_np.max()} reference_np: min{reference_np.min()} max{reference_np.max()}")
    print(f"Check the range of the input image: reconstructed_np: min{reconstructed_np.min()} max{reconstructed_np.max()} reference_np: min{reference_np.min()} max{reference_np.max()}")
    psnr_value = peak_signal_noise_ratio(reference_np, reconstructed_np, data_range=1)
    # print(f"psnr_value: {psnr_value}")
    # ssim_value = structural_similarity(reference_np, reconstructed_np, data_range=1, win_size = 11)
    # print(f"reference_np.shape: {reference_np.shape} reconstructed_np.shape: {reconstructed_np.shape}")
...
def get_noiselevel_alphas_timestep_del12012024(beta_at_clean, num_iters, schedule_name, num_diffusion_timesteps = 1000, last_time_step = 0, save_plot = False, save_root=None):
    #############
    # This code assume 1000 is the maximum number of time steps
    #############
    scale = 1000 / num_diffusion_timesteps
    assert scale == 1

    #############
    # Obtain pretrained score function's configuration
    #############
    ...
    #############
    # Set K' which is introduced Demonstration of Proposed Theory in the paper. Larger K' allows more precise matching with the given noise level
    #############
    if schedule_name == "pretrained":
        # Pretrained schedule does not require K' since we don't have arbitrary noise level that we need to match with pretrained schedule.
        discrete_steps = num_iters
        extended_length = num_iters
        
    elif schedule_name in ["sigmoid", "linear"]:
        # In this implementation we set discrete_steps as follows. Feel free to increase or decrease.
        discrete_steps = 1000000
        extended_length = num_iters
        assert discrete_steps >= extended_length
    else:
        raise ValueError("Check the name of the schedule")

    #############
    # Interpolate noise level of pretrained score function
    #############
...
def get_noiselevel_alphas_timestep_11282024(noise_level_to_get_time, beta_at_clean, denoiser_network_type, num_diffusion_timesteps = 1000, last_time_step = 0, previous_time_idx_in_list = -1):
...
# ! HERE, I add my function
def get_noiselevel_alphas_timestep_array_11282024(noise_level_array_to_get_time, beta_at_clean, denoiser_network_type, num_diffusion_timesteps = 1000, last_time_step = 0):
    if denoiser_network_type == "vp_score":
        scale = 1000 / num_diffusion_timesteps
        assert scale == 1
